...
init python:
    ...
    def sdl_achv_add_to_dict(x, val):
        z = x.copy()
        if val != None:
            z.update({val : "True"})
        return z
    ...
    class sdl_achv_Engine:
        ...
        def set_available_chars(self, char_arr):
            self.available_chars = char_arr
    ...
    class sdl_achv_Route:
        def __init__(self, title, typology, icon_active, icon_inactive, achv_arr, sprite_who, sprite_clothes, char_mask=7, interface=sdl_achv_interface_default, func_in=None, func_out=None, completed=True):
            self.title = title
    ...
    class sdl_achv_Achievement:
        def __init__(self, icon, persistent, text, prerequisites, replay, sprite_emo=None, char_mask=7):
            self.icon = icon
...
screen sdl_achvlist_main(engine):
    ...
    if engine.get_cur_section() != "me" and engine.get_cur_section() != "va" and engine.get_cur_route() == sdl_achv_route_default:
...
screen sdl_achvlist_section(engine):
    ...
    for route in sdl_achv_section_to_routes[engine.get_cur_section()]:
        ...
        if engine.get_cur_route() == route:
            ...
            imagebutton xcenter (1091 + 117 * sdl_achv_x_shift) ycenter (165 + 69 * sdl_achv_y_shift):
                ...
                action [Function(engine.set_hovered_route, None), Function(engine.run_func_out), Function(engine.set_cur_route, route), Function(engine.run_func_in), Show("sdl_achv_sprite", Dissolve(0.5), route.get_sprite_who(), "smile", route.get_sprite_clothes())]
        $ sdl_route_count += 1
    ...
    if engine.get_hovered_route() != None:
        ...
        if not engine.get_hovered_route().is_completed():
        ...
        if (not engine.get_cur_route().is_completed()) and (engine.get_hovered_icon() == None):
            add "sdl_achv_need_route" xcenter 440 ycenter 675
...
screen sdl_achv_route(engine):
    ...
    for achv in engine.get_cur_route().get_achv_arr():
        if achv.check_mask(engine.get_cur_char().get_char_mask()):
            ...
            if getattr(persistent, achv.get_persistent()):
                ...
                imagebutton xcenter 780 ycenter (165 + 64 * sdl_achv_count):
                    focus_mask True
                    idle (achv.get_icon())
                    hovered [Function(engine.set_hovered_achv, achv), Show("sdl_achv_sprite", Dissolve(0.5), engine.get_cur_route().get_sprite_who(), achv.get_sprite_emo(), engine.get_cur_route().get_sprite_clothes())]
                    unhovered [Function(engine.set_hovered_achv, None), Show("sdl_achv_sprite", Dissolve(0.5), engine.get_cur_route().get_sprite_who(), "smile", engine.get_cur_route().get_sprite_clothes())]
    ...
    if engine.get_cur_char_idx() < (len(engine.get_available_chars()) - 1):
...
screen sdl_achv_sprite(who, emo, clothes):
    ...
    if who != None:
        if persistent.achv_sprite_emo_7dl:
