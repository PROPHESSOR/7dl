...
label alt_day2_event_music_club:
    ...
    if (alt_day2_convoy == 'dv_prep'):
        ...
        if len(list_voyage_7dl) > 1:    # Алиса не дождалась - заходим сами
            pause(1)
...
label alt_day2_event_square:
    $ persistent.sprite_time = "day"
    $ day_time()
    if (alt_day2_convoy == 'sl'):    # Второе посещение со Славей
        $ disable_current_zone_alt1()
        ...
        if 'cleaning_sl' in list_voyage_7dl:    # Пришёл после уборки
            ...
            if (alt_day2_convoy == 'sl_prep') and (len(list_voyage_7dl) < 2):    # Первое посещение со Славей
