label alt_day4_neu_begin:
    ...
    if alt_day3_dv_evening:
        ...
        elif (counter_sl_cl == 7) or (lp_sl >= 13) or ((alt_day3_dancing == 2) and (counter_sl_7dl == 4)):
...
label alt_day7_neu_begin:
    ...
    pause(1)
    if persistent.me_neu_bad and persistent.alt_binder:
        menu:
            "Проснуться":
                $ persistent.sprite_time = "day"
                $ day_time()
                ...
                $ persistent.sprite_time = "prolog"
                $ prolog_time()
                if persistent.me_neu_loki_neu or persistent.me_neu_dr_neu:
                    menu:
                        "Настоящее":
                            $ alt_chapter(7, u"Одиночка. Настоящее")
                            if loki:
                                call alt_day7_neu_loki_neu
                            else:
                                call alt_day7_neu_dr_neu
                        "Будущее":
