...
label alt_day2_event_clubs1:
    ...
    if (alt_day_binder != 1):
        if counter_sl_7dl >= 1:
            ...
            th "Он!"
    th "Пойду к кибернетикам." # <PROPHESSOR> Он может ещё не знать про кибернетиков.
    ...
    if counter_sl_7dl >= 1 and loki:
        ...
        # <PROPHESSOR> В варианте со Славей, он увидел только «что-то продолговатое, зелёное, трещащее на лету»
            ...
            if 'music_club' in list_voyage_7dl:
                ...
                menu:
                    "Улыбнуться":
                        ...
                        $ karma += 10
                        $ lp_mi += 1
                        me "Приятного аппетита!" # <PROPHESSOR> «— Приятного аппетита!» «— Спасибо. И тебе.» «— Приятного аппетита!»
    ...
    if persistent.mi_dj_true:
        menu:
            "Я неприязненно дёрнул плечом":
                ...
                play music music_7dl["apologize"] fadein 3
...
label alt_day2_slot_sl:
    ...
    th "Сейчас она тепло поблагодарит меня за оказанную помощь, проводит до калитки или ещё лучше — скажет, куда идти — и распрощается." # <PROPHESSOR> Слишком резкий переход мыслей
    ...
    if ('sports' in list_slavya_7dl):
        ...
        "…"
        # <PROPHESSOR> TODO: Дописать сцену
        with joff_l
    else:
