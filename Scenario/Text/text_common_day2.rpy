label alt_day2_begin:
    ...
    if alt_day_binder != 1:
        ...
        if herc or loki or (dr and (counter_sl_7dl >= 1)):
            ...
            me "Да нет, я…"
            mt "Если ты о содержимом карманов — кстати, весьма странном — оно в том же шкафу."
        else:
            mt "Если ты о содержимом карманов — кстати, весьма странном — оно в шкафу."
    else:
...
label alt_day2_bf:
    ...
    menu:
        ...
        "Сесть с Леной":
            play music _7DL.music("take_my_hand") fadein 1
            $ lp_un += 1
            $ alt_day2_bf_un = True
...
label alt_day2_inmusic:
    ...
    play music _7DL.music("what_am_i_doing_here") fadein 1
    $ renpy.pause(1)
    ...
    if (alt_day2_convoy == 'me') or (alt_day2_convoy == 'sl_prep') or (alt_day2_convoy == 'dv_rej'):
        $ lp_mi += 1
    ...
    menu:
        "Записаться в кружок":
            ...
            if herc:
                ...
                mi "Прекрасно! Значит, на гитаре! Заходи сегодня сразу, как освободишься. Или завтра, в любое время!"
            else:
                mi "И всё?"
...
label alt_day2_event_clubs1:
    ...
    if ('library' in list_voyage_7dl):
        ...
        me "Должно быть, Александр, руководитель кружка кибернетики?"
        $ meet('sh','Шурик')
    ...
    elif len(list_joined_clubs_7dl) > 0:
...
label alt_day2_dubstep2:
    ...
    play music _7DL.music("polyhymnia_intro") fadein 3
...
label alt_day2_sl_hyst:
    ...
    play music _7DL.music("areyouabully") fadein 3
...
label alt_day2_dinner:
    scene expression Noon("bg ext_square_day") with joff_r
    ...
    if alt_day2_loki_minijack and (alt_day2_convoy not in ('dv_prep', 'dv_rej')):
...
label alt_day2_grand_escape:
    ...
    "Именно с таким взглядом моя бывшая раскручивала меня на совершенно сумасшедший секс в тамбуре поезда, идущего междугородним рейсом…" # TODO: разобраться с бывшими
    ...
    us "Что?"
    me "Железная дорога!" # TODO: написать вариант для Семёна, который не был на пристани
    ...
    if alt_day1_me_d3_dv_feed or alt_day_binder != 1:
...
label alt_day2_siesta:
    ...
    "Кроме всего прочего, ну не бывает такого, чтобы в целом огромном лагере минимум на сто человек в памяти отпечатались лишь ребята из отряда."
    ...
    if (alt_day2_convoy == 'un'):
        ...
        if loki:
            "Тот «Я» во сне назывался моим именем, но был больше похож на того меня, который ещё не стал Локи."
        else:
        ...
        if herc:
            "Я чувствую к нему неприязнь, хоть он и называет себя моим именем."
        else:
        ...
        "Улыбаться и излучать тёплое счастье."
...
label alt_day2_eventEv_estrade1:
    ...
    play music _7DL.music("splin") fadein 5
...
label alt_day2_dream:
    ...
    if (alt_day2_date == 'un'): # TODO: написать варианты для un_loki, un_herc и un_fz
    ...
    if (alt_day2_date == 'us'):
        ...
        th "Славя…"
    elif (alt_day2_date == 'un') or (alt_day2_date == 'un_loki') or (alt_day2_date == 'un_herc') or (alt_day2_date == 'un_fz'):
        th "Лена…"
    ...
    if persistent.mi_dj_true:
        menu:
            "Я неприязненно дёрнул плечом":
                ...
                play music _7DL.music("apologize") fadein 3
...
label alt_day2_slot_dv:
    ...
    play music _7DL.music("alice_theme") fadein 3
    window hide
...
label alt_day2_slot_sl:
    ...
    $ set_mode_nvl()
    if herc:
        "Мне девятнадцать… Белые питерские ночи, я сижу на подоконнике своей квартиры, выходящей окнами на бывшую церковь, из которой советская власть сделала бассейн."
        ...
        "КМБ, приказ, горячая точка." # Это вообще нормально, что срочника в 2000-х гг. отправляют в горячую точку? ## Поправка: в конце 2000-х, в то время не было горячих точек, куда отправляли срочников. Поэтому мы имеем некий абстрактный военный конфликт, достаточно масштабный, что сил контрактной армии оказалось недостаточно.
