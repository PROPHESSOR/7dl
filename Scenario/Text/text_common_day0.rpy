label alt_day0_start_d:
    ...
    with fade
    $ renpy.pause(3, hard=True)
    "Мало-помалу удалось привести голову в порядок, раскидать по дальним тёмным полочкам всё, что наговорил незванный гость."
...
label alt_day0_start_h:
    ...
    "А узнал я об этом, лишь, когда вылил его к первому в сковородку."
    ...
    stop music fadeout 3
    "Как обычно, в моменты вынужденного безделья мало-помалу начинаешь углубляться в воспоминания."
    window hide
    $ volume(1.0, 'music')
    play music music_7dl["seven_summer_days"] fadein 3
    $ renpy.pause(3)
    ...
    "Если бы жизнь была действительно честна с нами и взаправду предлагала свободу выбора, а не её иллюзию."
    ...
    "Что было бы, если б я-настоящий мог поговорить со мной-ребёнком и рассказать, что самым важным навыком будет умение быстро бегать и стрелять на шорох?"
    ...
    with fade
    play sound_loop sfx_street_traffic_outside fadein 2
    "Может быть, тогда не произошло бы и суда, и первого условного за нанесение средних телесных ответственному за мой вопрос офицеру."
    ...
    "У меня даже начало получаться и я стал даже немного зарабатывать сверх минимума."
    ...
    "Я закрыл глаза."
    $ renpy.pause(1.5)
    "В салоне играло что-то такое тёплое и доброе, родом из далёкого-далёкого детства."
    "Когда ещё не имели значения громкие имена и пафосный антураж, и народ просто тянулся к звукам хорошей песни."
    "Она навевала какую-то ностальгию, и изнутри отозвалось неожиданным теплом, разбежавшимся мурашками по коже, хоть немного, но всё же согревая."
    window hide
    stop music fadeout 3
    hide blink
    show unblink
    with dissolve
    $ renpy.pause(1.5)
    "Глаза сами открылись, стоило автобусу подъехать к моей остановки."
    window hide
    stop sound_loop
    ...
    "Несостоявшийся музыкант, несостоявшийся спортсмен и просто замечательный человек охраняет чужие сигареты, потому что ничего другого он просто не умеет."
    "Но, как я уже говорил, сегодня всё было не так."
    "Особенно стало не так, когда в помещение магазина, впустив вместе с собой целый клуб морозного воздуха, ввалился этот пьяный мудак."
    "И плевать он хотел на закон — ему хотелось выпить."
    "Я ожидал угроз, агрессии, мата. {w}В конце концов, затем я здесь и сижу."
    ...
    $ night_time()
    play ambience ambience_camp_center_night fadein 2
    "Две пули, два кусочка питерского неба — прямо в голову."
...
label alt_day0_d3_prologue:
    ...
    "И так если поглядеть по сторонам — то будто и не было этих сорока трёх лет."
    ...
    "Здесь каждое важное место покрашено своим личным цветом-номером, и почти можно расслышать смех тех, что однажды прибыли сюда, и…"
    ...
    "Её звали Алиса, но я звал её ангелом. {w}Это самое малое, как я мог отблагодарить за собственное спасение."
    ...
    th "Правду…"
    dreamgirl "Я ведь не вру. Тебе совершенно необязательно жить с чувством вины, или, тем более, пытаться что-то исправить, когда тебя ждут целые плеяды версий одного и того же события."
    ...
    "А не в посёлке Радченко Тверской области."
    "Так что же?"
    window hide
    scene bg ext_gostinka_night_7dl
    with dissolve
    ...
    "Я проводил её взглядом — она подбежала к куче детишек с транспарантом «ЛТО ЛУГА 2019»." # Не сходится. 1974 (год основания) + 43 (упоминается выше) = 2017 
    "Молодая вожатая."
    ...
    "Тот самый, кого я однажды видел повисшего над табуреткой."
...
label alt_day0_epic_fail:
    ...
    if loki:
        ...
        "Прошлое — зияющая рана. {w}Единственная твоя надежда на спасение — это обернуться и встретить его лицом к лицу."
